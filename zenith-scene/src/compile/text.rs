...
/// "Noto Sans". Extracted from [`compile_text`] so the table content-measurer
/// resolves families through the EXACT same path (single source of truth).
pub(super) fn resolve_text_families(
    text: &TextNode,
    resolved: &BTreeMap<String, ResolvedToken>,
    style_map: &BTreeMap<&str, &Style>,
    fonts: &dyn FontProvider,
...
pub(super) fn measure_text_natural(
    text: &TextNode,
    families: &[String],
    env: &MeasureEnv,
    diagnostics: &mut Vec<Diagnostic>,
...
pub(super) fn measure_text_wrapped_height(
    text: &TextNode,
    box_w: f64,
    families: &[String],
    env: &MeasureEnv,
